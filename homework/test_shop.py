...
class TestProducts:
    ...
    def test_product_buy(self, product):
        ...
        assert product.quantity ==1,'failed with equal quantity'
...
class TestCart:
    ...
    def test_buy(self, product, copybook, pencil, cart):
        ...
        assert cart.get_total_price() == 300.0, 'buy() method gives out not correct total_price'
        assert product.quantity == 999
        assert pencil.quantity == 996, 'product quantity not correct after buying cart'
        assert copybook.quantity == 998, 'product quantity not correct after buying cart'

    def test_buy_more_than_stock(self, product, cart):
         cart.add_product(product, product.quantity + 1)
         with pytest.raises(ValueError):
             assert cart.buy() is ValueError, 'failed with quantity more than stock'
